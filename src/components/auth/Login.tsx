...
export default function Login() {
  ...
  // Correction TypeScript : On définit le type de l'état de navigation
  const from = (location.state as { from?: string })?.from || '/dashboard';

  useEffect(() => {
    // CORRECTION DU BUG : On ajoute { replace: true } 
    // Cela évite de revenir en arrière sur la page de login une fois connecté
    if (state.user) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Utilisation d'Axios comme demandé
